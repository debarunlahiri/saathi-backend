...
public class CustomerController {
    ...
    public ApiResponse<CustomerProfile> save(@Valid @RequestBody CustomerProfileRequest request) {
        return ApiResponse.success("Customer profile saved successfully", customerService.save(request));
    }

    @GetMapping("/{customerId}")
    public ApiResponse<CustomerProfile> get(@PathVariable Long customerId) {
        return ApiResponse.success("Customer profile fetched successfully", customerService.get(customerId));
    }
}
