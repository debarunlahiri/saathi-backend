...
public class PaymentService {
    ...
    public Payment createOrder(CreatePaymentOrderRequest request) {
        ...
        long amountInPaise = request.amount().movePointRight(2).setScale(0, RoundingMode.HALF_UP).longValueExact();
    ...
    @Transactional public Payment verify(VerifyPaymentRequest request) {
        ...
        if (!gateway.verify(payment.getGatewayOrderId(), request.gatewayPaymentId(), request.signature())) {
    ...
    public long failedCount() { return repository.countByPaymentStatus(PaymentStatus.FAILED); }
}
