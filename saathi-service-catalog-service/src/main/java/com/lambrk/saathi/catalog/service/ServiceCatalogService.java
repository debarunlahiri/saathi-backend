...
public class ServiceCatalogService {
    ...
    public ServiceCatalogService(ServiceCategoryRepository repository) { this.repository = repository; }
    public List<ServiceCategory> active() { return repository.findByActiveTrue(); }
    public ServiceCategory save(ServiceCategoryRequest request, Long id) {
        ServiceCategory category = id == null ? new ServiceCategory() : repository.findById(id).orElseThrow();
