...
public class TaskApplicationService {
  ...
  public Task create(CreateTaskRequest request) {
    ...
    notificationClient.notifyUser(
        task.getCustomerId(),
        "Task created",
        "Your task is searching for a partner",
        "TASK_CREATED",
        task.getId().toString());
    return task;
  }
  ...
  public long activeTaskCount() {
    return taskRepository.countByTaskStatusIn(
        List.of(
  ...
  public Task assignNearest(UUID taskId) {
    UUID partnerId =
        assignmentStrategy
            ...
            .orElseThrow(
                () -> new IllegalStateException("No eligible partner available for this task"));
  ...
  public Task updateStatus(UUID taskId, TaskStatusRequest request) {
    ...
    if (request.status() == TaskStatus.TASK_COMPLETED) {
      ...
      if (task.getPartnerId() != null) {
        walletClient.createEarning(
            task.getId(),
            task.getPartnerId(),
            task.getEstimatedPrice(),
            DEFAULT_PLATFORM_COMMISSION);
        notificationClient.notifyUser(
            task.getPartnerId(),
            "Task completed",
            "Your earnings were added to wallet pending balance",
            "TASK_COMPLETED",
            task.getId().toString());
      }
      notificationClient.notifyUser(
